...
async function createChatCompletion(messages) {
    const tokens = messages.reduce((total, message) => {
        return (total +
            (0, helpers_1.getTokensFromText)(message.role) +
            (0, helpers_1.getTokensFromText)(message.content) +
            (0, helpers_1.getTokensFromText)(message.name));
    }, 0);
    if (tokens > config_1.default.openai.max_tokens) {
        throw new Error('The request has exceeded the token limit. Try again with a shorter message or start another conversation.');
    }
    try {
